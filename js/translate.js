...
function cambiarIngles() {
    ...
    document.getElementById("parrafoProjects_3").innerHTML = "This project arose from a challenge proposed by the Alkemy accelerator. This Android application allows you to see the list of the most popular movies, select them and see in detail a brief synopsis of the chosen movie, along with data such as the release date, language and genre. In addition, the app allows you to search for movies by title and be able to rate them. The app was developed in Android Studio with Kotlin."
    document.getElementById("parrafoProjects_4").innerHTML = "This project was the result of teamwork during acceleration with Alkemy. The app was developed for a fictitious case of \"ONG Somos Mas\", the application allows you to log in in three ways (traditional login, login with facebook, login with google). It also allows us to register, it has 6 screens where specific information about the NGO is displayed, and a contact screen to make inquiries. Loading systems and alert messages were implemented in each API query. This app was developed on Android with Kotlin, under an MVVM architecture, Material Design, Retrofit, dependency injection with Hilt, SharedPreferences and Firebase, among others, were implemented."
